...
for f in EXPECTED_FNS:
    if not hasattr(ex2, f):
        print(f, "is missing in ex2.py.")
        ex2.__dict__[f] = lambda *x: 0
...
class Clock(object):
    ...
    def __init__(self, tk, zone=None, **kargs):
        ...
        self.diff = ex2.hours_difference(dlt, 0)
    ...
    def refresh(self, forced=False):
        ...
        hour = int(ex2.time_from_utc(utc_hour, self.diff))
        ...
        minutes_angle = ex2.get_minutes(int(self.now)) / 60 * 360
        if ex2.time_from_utc(0, self.diff) % 1 == 0.5:
            minutes_angle += 180
        elif ex2.time_from_utc(0, self.diff) % 1 == 0.75:
            minutes_angle += 270
        ...
        seconds_angle = int(ex2.get_seconds(int(self.now))) / 60 * 360
...
if __name__ == '__main__':
    ...
    for x in range(4):
        canvases.append(Clock(frame,
                              zone=['local', 'UTC4.5', 'UTC+0.0', 'UTC+12.0'][x], row=0, column=x))
